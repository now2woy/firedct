...
public interface ItmTrdRepository extends JpaRepository<ItmTrd, ItmTrdPrimaryKey> {
	 ...
	 * @return
	 */
	public List<ItmTrd> findByItmCd(String itmCd, Pageable page);
	
	/**
	 * 종목코드, 일자에 해당하는 거래정보 조회
	 * @param itmCd
	 * @param dt
	 * @return
	 */
	public Optional<ItmTrd> findByItmCdAndDt(String itmCd, Date dt);
	 ...
	 * @return
	 */
	public List<ItmTrd> findByItmCdAndDtLessThanEqual(String itmCd, Date dt, Pageable page);
	...
	/**
	 * 입력일자보다 크거나 같은 일자 중 가장 빠른 날자 조회
	 * @param dt
	 * @return
	 */
	@Query("SELECT MIN(i.dt) AS dt FROM ItmTrd i WHERE dt >= :dt")
	public Date findMinDtByDt(@Param("dt") Date dt);
